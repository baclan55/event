...
// Keep-alive: пингует /api/health на Render, чтобы free-тир не засыпал
// и Neon не уходил в scale-to-zero между визитами. Cron Trigger в
// Cloudflare Dashboard: */5 * * * * (каждые 5 минут).
async function keepAlive() {
  const res = await fetch(`${ORIGIN}/api/health`, {
    method: 'GET',
    headers: { 'user-agent': 'event-denver-keepalive/1' },
  });
  if (!res.ok) {
    console.error('[keepalive] health returned', res.status);
  }
}
