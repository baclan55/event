...
async function login(discordUser: { id: string; username: string; avatar?: string | null }, session: Awaited<ReturnType<typeof getSession>>) {
  ...
  if (existing.rows[0]) {
    ...
    const inserted = await query<{ id: number }>('INSERT INTO users(discord_id,discord_username,nickname,role_id,is_owner,is_admin,avatar_url) VALUES($1,$2,$2,$3,$4,$4,$5) RETURNING id', [discordUser.id, discordUser.username, role.rows[0]?.id ?? null, grant, avatarUrl]);
...
export async function handle(key: string, request: NextRequest, context: Ctx): Promise<Response> {
  ...
  try {
    ...
    if (key === 'my-avatar' || key === 'roster-avatar') return jsonError('Аватар автоматически синхронизируется с Discord.', 410);
