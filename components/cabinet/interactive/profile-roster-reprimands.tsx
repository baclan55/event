    ...
    <>
      <div className="card card-pad profile-hero">
        ...
        <div className="profile-weekly-group">
          <div className="profile-weekly profile-stat">
            ...
            <div className="stat-label">Ð¼Ð¿ Ð·Ð° Ð½ÐµÐ´ÐµÐ»Ñ (Ð¿Ð½âÐ²Ñ)</div>
            {hasNorm ? (
              <span className={`badge ${done ? 'badge-green' : 'badge-red'}`}>
                {done ? 'Ð½Ð¾ÑÐ¼Ð°' : `ÑÐµÐ»Ñ ${target}`}
              </span>
            ) : (
              <span className="badge badge-muted">Ð±ÐµÐ· Ð½Ð¾ÑÐ¼Ñ</span>
            )}
          </div>
          <div className="profile-weekly profile-stat">
      ...
      <div className="card card-pad profile-body">
        <div className="segmented roster-tabs profile-tabs">
          ...
          {tabs.events ? (
            <button className={tab === 'events' ? 'active' : ''} onClick={() => setTab('events')}>ÐÐµÑÐ¾Ð¿ÑÐ¸ÑÑÐ¸Ñ Â· {eventWeekCount}/{eventTotal}</button>
          ) : null}
          {tabs.gmp ? (
        ...
        {tab === 'reprimands' ? (
          ...
          <>
            <div className="card-header"><h3>{isSelf ? 'ÐÐ¾Ð¸ Ð¼ÐµÑÐ¾Ð¿ÑÐ¸ÑÑÐ¸Ñ' : 'ÐÐµÑÐ¾Ð¿ÑÐ¸ÑÑÐ¸Ñ'}</h3><span className="badge badge-muted">{eventTotal}</span></div>
            <div className="field-hint" style={{ marginBottom: 10 }}>Ð¢Ð¾Ð»ÑÐºÐ¾ Ð¿ÑÐ¾Ð²ÐµÐ´ÑÐ½Ð½ÑÐµ Â· Ð·Ð° Ð½ÐµÐ´ÐµÐ»Ñ {eventWeekCount}</div>
            {eventItems.length ? eventItems.map((item) => (
              <div className="roster-row" key={String(item.message_id)}>
                <div className="who">
                  <div>
                    <div className="nickname">{String(item.title || 'ÐÐµÐ· Ð½Ð°Ð·Ð²Ð°Ð½Ð¸Ñ')}</div>
                    <div className="role-tag">{new Date(String(item.message_created_at)).toLocaleString('ru-RU')}{item.event_key ? ` Â· ${item.event_key}` : ''}</div>
                  </div>
                </div>
                <span className="badge badge-green">ÐÑÐ¾Ð²ÐµÐ´ÐµÐ½Ð¾</span>
              </div>
            )) : <div className="empty-state"><h3>ÐÐµÑÐ¾Ð¿ÑÐ¸ÑÑÐ¸Ð¹ Ð½ÐµÑ</h3><p>ÐÑÐ¾Ð²ÐµÐ´ÑÐ½Ð½ÑÑ ÑÐ±Ð¾ÑÐ¾Ð² ÐÐ Ð¿Ð¾ÐºÐ° Ð½ÐµÑ.</p></div>}
            {eventTotal > 0 ? (
              <div className="toolbar" style={{ marginTop: 12, marginBottom: 0 }}>
                <div className="toolbar-left">Ð¡ÑÑÐ°Ð½Ð¸ÑÐ° {eventPage} Ð¸Ð· {eventTotalPages}</div>
    ...
    <div className="roster-row" key={member.id}>
      ...
      {candidate ? <span className="badge badge-amber">ÐÐ¶Ð¸Ð´Ð°ÐµÑ Ð¾Ð±Ð·Ð²Ð¾Ð½Ð°</span> : <>
        ...
        {(() => {
          ...
          if (norm == null) return <span className="badge badge-muted">{count} / Ð½ÐµÐ´.</span>;
  ...
  return (
    <>
      <div className="toolbar">
        <div className="toolbar-left">{members.length} ÑÑÐ°ÑÑÐ½Ð¸ÐºÐ¾Ð² Â· Ð½Ð¾ÑÐ¼Ð° Ð¿Ð¾ ÑÐ¾Ð»ÑÐ¼ (Ð½ÐµÐ´ÐµÐ»Ñ Ð¿Ð½âÐ²Ñ)</div>
      ...
      {editing !== undefined && (
        <Modal title={editing ? 'Ð ÐµÐ´Ð°ÐºÑÐ¸ÑÐ¾Ð²Ð°Ð½Ð¸Ðµ ÑÑÐ°ÑÑÐ½Ð¸ÐºÐ°' : 'ÐÐ¾Ð²ÑÐ¹ ÑÑÐ°ÑÑÐ½Ð¸Ðº'} onClose={() => setEditing(undefined)} wide>
          <form onSubmit={saveMember}>
            ...
            <div className="field"><label>Discord ID</label><input className="input" name="discordId" inputMode="numeric" placeholder="Ð´Ð»Ñ Ð¿ÑÐ¸Ð²ÑÐ·ÐºÐ¸ ÐÐ Ð´Ð¾ Ð²ÑÐ¾Ð´Ð° Ð½Ð° ÑÐ°Ð¹Ñ" defaultValue={editing?.discord_id || ''} /><div className="field-hint">ÐÑÐ»Ð¸ ÑÐºÐ°Ð·Ð°ÑÑ Discord ID Ð·Ð°ÑÐ°Ð½ÐµÐµ, ÑÑÐ°ÑÑÐµ ÑÐ±Ð¾ÑÑ ÐÐ Ð¿Ð¾Ð´ÑÑÐ½ÑÑÑÑ Ðº Ð¿ÑÐ¾ÑÐ¸Ð»Ñ Ð¿Ð¾ÑÐ»Ðµ Ð¿ÑÐ¸Ð²ÑÐ·ÐºÐ¸ Ð¸Ð»Ð¸ Ð²ÑÐ¾Ð´Ð°.</div></div>
